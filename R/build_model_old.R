...
# colnames(subacc_wide_only_dead) <- new_col_names
...
create_ggheatmap <- function(metled_corr) {
...
calculate_corr_and_print_heatmap <- function(subacc) {
  ...
  ggheatmap <- create_ggheatmap(melted_cormat)
  print(ggheatmap)
}

calculate_corr_and_print_heatmap(subacc_wide)
calculate_corr_and_print_heatmap(subacc_wide_only_dead)
...
mycors_only_dead <- cor(as.matrix(subacc_wide_only_dead[,cols_subset]))
# Reorder the correlation matrix
cormat_only_dead <- reorder_cormat(mycors_only_dead)
cormat_only_dead[lower.tri(cormat_only_dead)] <- NA
# upper_tri <- get_upper_tri(mycors)
# Melt the correlation matrix
melted_cormat_only_dead <- melt(cormat_only_dead, na.rm = TRUE)
ggheatmap_only_dead <- create_ggheatmap(melted_cormat_only_dead)
